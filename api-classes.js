...
class StoryList {
  ...
  /**
   * Method to delete a story and update storyList
   *
   * - user: the currenter User
...
class User {
  ...
  /**
   * This function fetches the user api and sets all properties,
   * generally used to update the user instance on the fly
   */
   ...
   async retrieveDetails() {
    ...
    //update user's properties from API
  ...
  /**
   *  add Favorite story to the User api
   */
  async addFavoriteStory(username, storyId, token) {
    ...
    this.favorites = [];
    let favorites = res.data.user.favorites.map(fav => {
      return fav;
    });
  ...
  /**
   * delete Favorite story from the User api
   */

  async deleteFavoriteStory(username, storyId, token) {
